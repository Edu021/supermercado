                    ...
                    <div class="modal-dialog">
                      <div class="modal-content">
                        ...
                        <div class="modal-footer">
                          ...
                          <form action="http://3.21.113.127/supermercado/api/api_pedidos_compra.php?finalizar=${cliente.pedidos[i].codigo}" method="POST">
...
function syncaPedidosComItens() {
  ...
  for (i in pedidos) {
    ...
    for (j in itens) {
      if (pedidos[i].id_pedido == itens[j].id_pedido_compra) {
        objCliente.empresa = pedidos[i].empresa;
        objPedidos.codigo = pedidos[i].id_pedido;
        objPedidos.status = pedidos[i].status;
        objPedidos.cnpj = pedidos[i].cnpj;
        objPedidos.cep = pedidos[i].cep;
        objPedidos.numero = pedidos[i].numero;
        objPedidos.valor = itens[j].vlr_unit*itens[j].quant_produto;
        objPedidos.descricao.push(itens[j].nm_produto + " / " + itens[j].quant_produto + " un. \n");
      }
    }
